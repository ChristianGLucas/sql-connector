...
// empty is a structured error. This node is a kind:generic template — this
// function never runs against caller-chosen input directly; the Instance
// that binds this template to a real database pins ConnectionConfig itself
// (a literal `dsn` constant for a public database, or a `dsn_secret_name`
// matching a name the Instance declares in its own required_secrets for a
// private one) — see ConnectionConfig's doc comment for why a bare,
// undeclared secret name would NOT be deliverable outside that structure.
